...
README_CONTENT_CHECK_FOR = [
    ...
    'speed_converter',
    '*args', '**kwargs',
    'repetitions', 'temp_given_in', 'side', 'length', 'area']
...
def test_session5_readme_500_words():
    """ Checks if README.md file has at least 500 characters, raises assert error otherwise
    """
...
def test_session5_readme_proper_description():
    """ Checks README.md file for description of all important function names and arguments.

    Includes: 'time_it(fn, *args, repetitions= 1, **kwargs)',
    'squared_power_list',
    'polygon_area',
    'temp_converter',
    'speed_converter',
...
def test_session5_readme_file_for_more_than_10_hashes():
    """ Checks if README has decent formatting and uses at least 10 hashes (10 #s and not 🚬 )
    """
...
def test_session5_time_it_print():
    """ Uses time_it function to test in-built `print` function,
        with repetitions and arguments (both positional as well as keyword) to be passed to print,
        the return type must be float (average time for run)
    """
    assert isinstance(session5.time_it(print, 1, 2, 3, 4, repetitions=5, end="***"),
                      float), "time_it can't time print function"


def test_session5_time_it_squared_power_list():
    """
    Uses time_it function to test `squared_power_list` function,
    with repetitions and arguments (both positional as well as keyword) to be passed,
    the return type must be float (average time for run)
    """
    assert isinstance(session5.time_it(session5.squared_power_list, number=2, repetitions=5, start=1, end=4),
                      float), "time_it can't time squared_power_list function"


# def test_session5_time_it_squared_power_list_multi():
#     """
#     Uses time_it function to test `squared_power_list` function, with multiple positional arguments for
#     squared_power_list, with repetitions and arguments (both positional as well as keyword) to be passed,
#     the return type must be float (average time for run)
#     """
#     assert isinstance(session5.time_it(session5.squared_power_list, 1, 2, 3, 4, repetitions=5, start=1, end=10),
#                       float), "time_it can't time squared_power_list function"


def test_session5_time_it_polygon_area():
    """
    Uses time_it function to test `polygon_area` function,
    with repetitions and arguments (both positional [length] as well as keyword [side]) to be passed,
    the return type must be float (average time for run)
    """
    assert isinstance(session5.time_it(session5.polygon_area, 4, repetitions=5, sides=6),
                      float), "time_it can't time polygon_area function"


# def test_session5_time_it_polygon_area_multi():
#     """
#     Uses time_it function to test `polygon_area` function, with multiple positional arguments
#     with repetitions and arguments (both positional as well as keyword) to be passed,
#     the return type must be float (average time for run)
#     """
#     assert isinstance(session5.time_it(session5.polygon_area, 4, 2, 1, 10, repetitions=5, side=5),
#                       float), "time_it can't time polygon_area function"


def test_session5_time_it_speed_converter():
    """
        Uses time_it function to test `speed_converter` function,
        with repetitions and arguments (both positional [speed] as well as keyword [dist_type, time_type]) to be passed,
        the return type must be float (average time for run)
        """
    assert isinstance(session5.time_it(session5.speed_converter, 100, repetitions=5, dist='m', time='min'),
                      float), "time_it can't time speed_converter function"


# def test_session5_time_it_speed_converter_multi():
#     """
#         Uses time_it function to test `speed_converter` function, with multiple positional arguments
#         with repetitions and arguments (both positional [speed] as well as keyword [dist_type, time_type]) to be passed,
#         the return type must be float (average time for run)
#         """
#     assert isinstance(session5.time_it(session5.speed_converter, 110.14, 115, 135, 147, 160,
#                                        repetitions=5, dist_type='m', time_type='min'),
#                       float), "time_it can't time speed_converter function"


def test_session5_time_it_temp_converter():
    """
    Uses time_it function to test `temp_converter` function,
    with repetitions and arguments (both positional [temp] as well as keyword [temp_given_in]) to be passed,
    the return type must be float (average time for run)
    """
    assert isinstance(session5.time_it(session5.temp_converter, 37.5, repetitions=5, temp_given_in='c'),
                      float), "time_it can't time temp_converter function"


# def test_session5_time_it_temp_converter_multi():
#     """
#     Uses time_it function to test `temp_converter` function, with multiple positional arguments
#     with repetitions and arguments (both positional [temp] as well as keyword [temp_given_in]) to be passed,
#     the return type must be float (average time for run)
#     """
#     assert isinstance(
#         session5.time_it(session5.temp_converter, 97.2, 98.5, 100.4, repetitions=5, temp_given_in='f'),
#         float), "time_it can't time temp_converter function"
...
def test_session5_time_it_zero_rep():
    ...
    assert session5.time_it(squared_power_list, 2, start=0, end=5,
                            repetitions=0) == 0, "time_it should retun 0 for no function call"
    assert session5.time_it(polygon_area, 15, sides=3,
                            repetitions=0) == 0, "time_it should retun 0 for no function call"
    assert session5.time_it(speed_converter, 100, dist='km', time='min',
                            repetitions=0) == 0, "time_it should retun 0 for no function call"
    assert session5.time_it(temp_converter, 100, temp_given_in='f',
                            repetitions=0) == 0, "time_it should retun 0 for no function call"
...
def test_session5_polygon_area_length():
    """Test polygon_area function for incorrect values for positional argument length (check for string AND imaginary
    input)"""
    with pytest.raises(TypeError, match=r".*Only integer type arguments are allowed*"):
        session5.polygon_area('hulala', sides=4)
    with pytest.raises(TypeError, match=r".*Only integer type arguments are allowed*"):
        session5.polygon_area(1 + 2j, sides=4)
...
def test_session5_polygon_area_sides():
    ...
    with pytest.raises(TypeError, match=r".*Only integer type arguments are allowed*"):
        session5.polygon_area(2, sides="ten")
    with pytest.raises(TypeError, match=r".*Only integer type arguments are allowed*"):
        session5.polygon_area(2, sides=1 + 10j)


def test_session5_polygon_area_sides_values():
    """Test polygon_area function for permissible values for sides, check for 0, 1, 2, 7"""
    with pytest.raises(ValueError, match=r".*check value of sides given, must be in range 3,6 inclusive*"):
        session5.polygon_area(4, sides=0)
    with pytest.raises(ValueError, match=r".*check value of sides given, must be in range 3,6 inclusive*"):
        session5.polygon_area(length=2, sides=1)
    with pytest.raises(ValueError, match=r".*check value of sides given, must be in range 3,6 inclusive*"):
...
def test_session5_polygon_area_length_values():
    ...
    with pytest.raises(ValueError, match=r".*'length' should be positive*"):
...
def test_session5_polygon_area_unwanted_args():
    ...
    with pytest.raises(TypeError,
                       match=r".*polygon_area function takes maximum 1 positional arguments, more provided*"):
        session5.polygon_area(1, 2, sides=4)
    with pytest.raises(TypeError,
                       match=r".*polygon_area function take maximum 1 keyword/named arguments, more provided*"):
...
def test_session5_temp_converter_temp():
    """Test temp_converter function for incorrect values for positional argument temp (check for string AND imaginary
    input) """
    with pytest.raises(TypeError, match=r".*Only float/int type arguments are allowed*"):
        session5.temp_converter('hulala')
    with pytest.raises(TypeError, match=r".*Only float/int type arguments are allowed*"):
...
def test_session5_temp_converter_temp_values_in_celsius():
    """ DON'T TOUCH THIS FUNCTION \
        Test temp_converter function for permissible values for temperature in celsius"""
...
def test_session5_temp_converter_unwanted_args():
    ...
    with pytest.raises(TypeError,
                       match=r".*temp_converter function takes maximum 1 positional arguments, more provided*"):
        session5.temp_converter(100, 2, temp_given_in='F')
    with pytest.raises(TypeError,
                       match=r".*temp_converter function take maximum 1 keyword/named arguments, more provided*"):
...
def test_session5_speed_converter_unwanted_args():
    ...
    with pytest.raises(TypeError,
                       match=r".*speed_converter function takes maximum 1 positional arguments, more provided*"):
        session5.speed_converter(100, 2)
    with pytest.raises(TypeError,
                       match=r".*speed_converter function take maximum 2 keyword/named arguments, more provided*"):
